...
// 올바른 인자 순서 사용
var discountedProducts = products.map(function (product, index, array) {
  console.log(
    "처리 중: " + (index + 1) + "/" + array.length + " - " + product.name,
  );
  return {
    name: product.name,
 ...
 */
var wrongResult = products.map(function (index, product) {
...
function onUploadProgress(progress, uploaded, total) {
  var bar = "";
  for (var i = 0; i < progress / 5; i++) {
    bar += "█";
  }
  for (var j = bar.length; j < 20; j++) {
    bar += "░";
  }
